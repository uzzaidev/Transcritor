...
# -*- coding: utf-8 -*-
"""
Script para transcrever os 3 arquivos de reunião do carro
Transcreve: Reuniao pai carro 1.m4a, Reuniao pai carro 2.m4a, Reuniao pai carro 3.m4a
...
ARQUIVOS = [
    "Reuniao pai carro 1.m4a",
    "Reuniao pai carro 2.m4a",
    "Reuniao pai carro 3.m4a"
]
    ...
    try:
        ...
        if resultado and resultado.get('text'):
            ...
            with open(arquivo_saida, 'w', encoding='utf-8') as f:
                f.write(f"🎵 TRANSCRIÇÃO - REUNIÃO CARRO\n")
                f.write(f"=" * 80 + "\n")
            ...
            print(f"\n📝 TEXTO TRANSCRITO:")
            print(f"{'-'*80}")
            print(texto[:500] + "..." if len(texto) > 500 else texto)  # Mostra apenas os primeiros 500 caracteres
            print(f"{'-'*80}")
...
def main():
    """
    Função principal para transcrição dos 3 arquivos de reunião
    """
    print("🎯 TRANSCRITOR DE REUNIÕES - REUNIÃO CARRO")
    print("=" * 80)
    ...
    if transcricoes:
        ...
        arquivo_consolidado = os.path.join(pasta_atual, f"reuniao_carro_completo_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
        
        with open(arquivo_consolidado, 'w', encoding='utf-8') as f:
            f.write("🚗 REUNIÃO CARRO - TRANSCRIÇÕES COMPLETAS\n")
            f.write("=" * 80 + "\n")
