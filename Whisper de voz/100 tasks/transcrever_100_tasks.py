...
# -*- coding: utf-8 -*-
"""
Script para transcrever arquivos MP3 da pasta 100 tasks
Transcreve 6 arquivos .mp3 específicos
...
# Lista dos 6 arquivos específicos na ordem
ARQUIVOS = [
    "22 - 100 TASKS.mp3",
    "37 - 100 tasks.mp3",
    "75.mp3",
...
def verificar_ffmpeg():
    ...
    Necessário para processar arquivos .mp3
    """
    if shutil.which("ffmpeg") is None:
        ...
        print("💡 O ffmpeg é necessário para converter arquivos .mp3")
        return False
    return True

def transcrever_arquivo(model, caminho_arquivo, pasta_saida, device, indice, total):
    """
    Transcreve um arquivo de áudio .mp3 usando configurações otimizadas
    ...
    if tamanho == 0:
        print(f"❌ Arquivo está vazio (0 bytes)!")
        return False, ""
    
    print(f"📊 Tamanho do arquivo: {tamanho / (1024 * 1024):.2f} MB")
    ...
    try:
        ...
        if resultado and resultado.get('text'):
            ...
            with open(arquivo_saida, 'w', encoding='utf-8') as f:
                f.write(f"🎵 TRANSCRIÇÃO DE ÁUDIO - 100 TASKS\n")
                f.write(f"=" * 80 + "\n")
...
def main():
    """
    Função principal para transcrição dos arquivos 100 tasks
    """
    print("🎯 TRANSCRITOR DE ÁUDIO - 100 TASKS")
    print("=" * 80)
    
    # Verificar ffmpeg (essencial para .mp3)
    ...
    for arquivo in ARQUIVOS:
        ...
        if os.path.exists(caminho_completo):
            ...
            if tamanho > 0:
                ...
                print(f"⚠️ Arquivo vazio (0 bytes): {caminho_completo}")
        else:
    ...
    if transcricoes:
        ...
        arquivo_consolidado = os.path.join(pasta_atual, f"100_tasks_completo_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
        
        with open(arquivo_consolidado, 'w', encoding='utf-8') as f:
            f.write("💬 100 TASKS - TRANSCRIÇÕES COMPLETAS\n")
            f.write("=" * 80 + "\n")
            f.write(f"📅 Data de processamento: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"📊 Total de arquivos: {len(transcricoes)}\n")
            f.write("=" * 80 + "\n\n")
            
            for trans in transcricoes:
                f.write(f"\n{'─'*80}\n")
                f.write(f"🎵 ARQUIVO {trans['indice']}: {trans['arquivo']}\n")
                f.write(f"{'─'*80}\n")
