...
# -*- coding: utf-8 -*-
"""
Script para transcrever 3 reuniões da semana sprint (versão 2)
Transcreve: erika incubadora (1).m4a, Luiscasa (1).m4a, luciano (1).m4a
...
# Lista dos 3 arquivos específicos na ordem
ARQUIVOS = [
    "erika incubadora (1).m4a",
    "Luiscasa (1).m4a",
    "luciano (1).m4a"
]
...
def transcrever_arquivo(model, caminho_arquivo, pasta_saida, device, indice, total):
    ...
    try:
        ...
        if resultado and resultado.get('text'):
            ...
            # Remover "(1)" do nome do arquivo de saída
            nome_base = nome_base.replace(" (1)", "")
...
def main():
    """
    Função principal para transcrição das 3 reuniões
    """
    print("🎯 TRANSCRITOR DE REUNIÕES - SEMANA SPRINT (VERSÃO 2)")
    print("=" * 80)
    ...
    if transcricoes:
        ...
        arquivo_consolidado = os.path.join(pasta_atual, f"reunioes_completas_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
        
        with open(arquivo_consolidado, 'w', encoding='utf-8') as f:
            f.write("💬 REUNIÕES SEMANA SPRINT - TRANSCRIÇÕES COMPLETAS (VERSÃO 2)\n")
            f.write("=" * 80 + "\n")
